...
module.exports = function laundryRouter() {
    router.post('/addReminder', async (req, res) => {
        let { subscription, machineID, hallID, hallName, reminderID } = req.body;
        ...
        setTimeout(async () => {
            try {
                await webpush.sendNotification(subscription, JSON.stringify({ machineID, hallID, hallName, reminderID }));
                res.status(200).json({})
